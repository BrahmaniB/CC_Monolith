...
def browse():
    try:
        # Get page number from query parameter and validate it
        ...
        products_to_display = products.get_products_paginated(page, products_per_page)

        # If no products are found, return an empty list
        if not products_to_display:
            products_to_display = []

        return render_template('browse.jinja', items=products_to_display, srn=SRN, page=page)

    except Exception as e:
        # Log the error for debugging
        print(f"Error in /browse: {e}")
        return "An error occurred while loading the browse page.", 500
